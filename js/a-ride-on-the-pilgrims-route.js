...
$(document).ready(function(){
  ...
  var blogURL     = '/blogs/a-ride-on-the-pilgrims-route';
  var blogTitle   = "A Ride on The Pilgrims' Route";
  ...
  // LOAD BLOG SUMMARY IF NOT ALREADY LOADED -----------------------------

  if(window.location.href===blogURL){
    
    blogContent = blogPannel.clone(true);
    main(blogContent);

  }else{$.get(blogURL).done( function(text){
    
    blogContent = $(text).filter('main').find('#blog-pannel') ;
    main(blogContent);
  ...
  function main(blogContent){
    ...
    map.on('load', function(){
      blogPannel.trigger('state:update', {
        mapbounds: bounds2array( map.getBounds() )
      });
    });
    ...
    window.onpopstate = function(e){
      ...
      if(typeof e.state !== "undefined"){
        if(typeof e.state.title     !== "undefined") document.title = e.state.title;
        if(typeof e.state.content   !== "undefined") blogPannel.html(e.state.content);
        if(typeof e.state.scroll    !== "undefined") blogPannel.scrollTop(e.state.scroll);
        if(typeof e.state.mapbounds !== "undefined") map.flyToBounds(
          array2bounds(e.state.mapbounds),
          { maxZoom: 10 }
        );
        blogPannel.trigger('post:restore', e.state.selected);
      console.log(e.state.scroll);
      }
    };
    ...
    function chargeDailyTracks( thePilgrimsRoute ){
      ...
      ids.each(function(index, id){
        ...
        .done(function( data ){
          ...
          // 3. handle the case where a post is loaded directly

          var pathname = blogContent.find('#'+id+' > a').attr('href');
          if(window.location.pathname === pathname){
            track.setStyle({opacity: 1});
            map.flyToBounds(track);
            blogPannel.trigger('state:update', {
    ...
    blogPannel.on( 'click', 'a', function(e){
      ...
      blogScroll = blogPannel.scrollTop();
      blogPannel.trigger('state:update', {scroll: blogScroll});

      $.get(postURL).done(function(text){

        var html        = $(text);
        var postTitle   = html.filter('title').text() ;
        var postContent = html.filter('main').find('#blog-pannel > *') ;
